...
**NA means not available.This could occur due to insufficient value in the data set. There are few reasons why this could happened, maybe the person in charge had missed a few data or there are other issues. In order for us to calculate mean and median, there must be sufficient data.**
...
**Based on the help page on the bottom right panel, 'na.rm' means a logical evaluating to TRUE and FALSE indicating whether NA values should be stripped before the computation proceeds. If it is true the NA will be stipped.**
...
**Mean is the average data that is calculated by adding all values together and divide it by the total. Median is the middle value in the data which is calculated by arranging the data from smallest to largest and the middle one is the median. Mean is known to be easily affected by outliers or known as extreme values. Therefore, higher value of means may be caused by existence of extreme values.**
